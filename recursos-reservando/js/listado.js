...
Listado.prototype.buscarRestaurante = function(id) {
    ...
    //-----REFACTORIZADA uso con Filter
   
    var resto = this.restaurantes.filter(restaurant=>restaurant.id===id)
...
Listado.prototype.obtenerUbicaciones = function() {
   ...
   //---------REFACTORIZADO cambiando el for por un map
    ...
    //Se crea un nuevo array donde se van a agregar las ciudades pero sin repetirse
        var ciudad = eliminarRepetidos(ciudades)
...
Listado.prototype.obtenerRubros = function() {
    ...
    var rubroR = eliminarRepetidos(rubros)
...
Listado.prototype.obtenerHorarios = function() {
    ...
    //---------REFACTORIZADO cambiando el for por un map
    var arrayH = this.restaurantes.map(restaurante=>restaurante.horarios)

    //En este arreglo vamos a poner todos los horarios, uno por uno
    var horarios = [];
    arrayH.forEach(function(a) {
    ...
    var horaR = eliminarRepetidos(horarios)
...
function eliminarRepetidos(arr){
    ...
    var arr_sinRepetir = arr.filter(function(elem, index, self) {
        return index === self.indexOf(elem);
    });

    return arr_sinRepetir.sort();
}
